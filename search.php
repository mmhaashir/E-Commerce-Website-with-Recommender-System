...
<body>
	...
	<?php
		...
		$query = "SELECT * FROM `products` WHERE `product_title` LIKE CONCAT('%', ?, '%')";
		$stmt = $db->prepare($query);
		$row_count = 0;
		$col_count = 3;
		if($stmt->execute([$search_query]))
		{
			if($stmt->rowCount()<1)
			{
				echo("<div class='mx-auto'><img src='img/no-found.png' style='height: 300px; width: 500px;'></div>");
			}
